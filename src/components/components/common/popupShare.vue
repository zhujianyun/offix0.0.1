   ...
   <div class="share_box">
     <i class="popup_close iconfont icon-guanbi cur fr cancelIcon" @click="popupCancel"></i>
      <div class="share_left fl">
        <div class="share_btt fl">
          <button class="main_button_bg" type="button">手机分享</button>
          <button class="main_button_color" type="button">邮件分享</button>
        </div>
        <div class="share_phone fl">
          <div class="phone_page">
            <div id="shareImg" class="share_img">
              <img v-if="shareCoverChecked" src="../../assets/img/phone_content1.png" alt="" class="share_img_cover">
              <img v-if="!shareCoverChecked" src="../../assets/img/phone_content2.png" alt="" class="share_img_content">
              <share-page v-if="false" class="share_page"></share-page>
            </div>
          </div>
        </div>
        <div class="share_scroll fl">
     ...
     <div class="share_right fl">
       <div v-show="editPage" class="share_right_one">
       ...
       <div v-show="!editPage" class="share_right_two">
         ...
         <div class="share_link">
           ...
           <el-input class="share_link_input fl" id="shareLink" v-model="shareLink" placeholder="请输入内容"></el-input>
    ...
    export default {
      ...
      data() {
          return {
            ...
            staffList: [], // 协助人员列表
            editPage: true, // 分享编辑页
            shareLink: 'http://182.92.161.72:1011/#/myTesk/myTeskList', // 分享链接
      ...
      methods: {
        ...
        // 展示内容
        scrollUp() {
          this.shareImgMtop =  Number($('#shareImg').css('margin-top').replace('px', ''));
          ...
          $('#shareImg').animate({marginTop:this.shareImgMtop + scrollSize +"px"},200, () => {
            this.shareImgMtop = Number($('#shareImg').css('margin-top').replace('px', ''));
            this.shareImgMbottom = $('#shareImg').height() - $('#shareImg').parent().height() + this.shareImgMtop;
          });
        },
        ...
        scrollDown() {
          ...
          this.shareImgMtop =  Number($('#shareImg').css('margin-top').replace('px', ''));
          let scrollSize = 391;
          if(this.shareImgMbottom !== null && parseInt(this.shareImgMbottom) < scrollSize) {
        ...
        clickShare() {
          ...
          let obj = {title: this.shareTitle, articlePkid: this.shareData.Pkid, descn: this.shareRequire, isTaskFile: this.shareFlieChecked, coordinationsId: this.shareStaffList.join(','), author: this.userID, isTitle: this.shareCoverChecked, isDescn: this.shareRequireChecked };
          this.$HTTP('post', '/SetSharingLinks', obj).then(res => {
        ...
        staffChange(staff) {
          ...
          // console.log(111, staff, this.staffList);
        },

        // 获取卡片详情
        getCardDetail(teskId) {
          this.userID = JSON.parse(localStorage.getItem('staffInfo')).Pkid.toString();
          let loginName = JSON.parse(localStorage.getItem('staffInfo')).Realname;
          let obj = {articlePkid: teskId, userid: this.userID};
          this.$HTTP('post', '/GetArticle', obj, '.tesk_card').then( res => {
            ...
            if(this.userID !== this.shareData.UserId.toString() && this.shareData.UserPkids.indexOf(this.userID) !== -1) {
            ...
            this.fileList = this.shareData.PubFile ? JSON.parse(this.shareData.PubFile) : [];
      ...
      .share_left {
        ...
        .share_phone {
          ...
          .phone_page {
            ...
            height: 391px;
            overflow: hidden;
            // background: linear-gradient(blue,#450daa);
