  ...
  export default {
    ...
    data() {
      return {
        ...
        newsList1: [
          {
            ...
            Content: 0, // 消息类型
            Enabled: true,
            userPic: '/upload/user/6ce20ef2-4141-40e2-9597-4f41c081495a.png',
            CreateTime: '7月14日 10:42'
          }, {
            Pkid: '002',
            userName: '祝建云',
            Type: 0, // 卡片类型
            Content: 1, // 消息类型
            Enabled: true,
            userPic: '/upload/user/870637a4-e1d1-49fe-b288-7c506a042228.png',
            CreateTime: '7月14日 09:20'
          }, {
            Pkid: '003',
            userName: '卢洪臣',
            Type: 0, // 卡片类型
            Content: 2, // 消息类型
            Enabled: true,
            userPic: '/upload/user/b3a309cd-4379-4860-900b-1ced499032a9.png',
            CreateTime: '7月13日 19:23'
          }, {
            Pkid: '004',
            userName: '赵珂',
            Type: 0, // 卡片类型
            Content: 3, // 消息类型
            Enabled: true,
            userPic: '/upload/user/2a76eca0-fd67-4ad0-b2a5-c3e9b169659b.png',
            CreateTime: '7月13日 16:57'
          }, {
            Pkid: '005',
            userName: '袁沛豪',
            Type: 0, // 卡片类型
            Content: 4, // 消息类型
            Enabled: true,
            userPic: '/upload/user/67c56166-c8c4-406f-85fb-dcf2637136a4.png',
            CreateTime: '7月13日 13:26'
          }, {
            Pkid: '006',
            userName: '罗建鹏',
            Type: 0, // 卡片类型
        ...
        newsTypes: ['添加了反馈', '上传了文件', '完成了任务', '添加了新的评论', '给您创建了一个新的', '添加了新的需求', '删除了新的需求', '上传需求文件', '删除需求文件', '添加了参与者', '删除协同人', '删除回复内容', '撤回文件', '修改完成状态', '修改任务时间'], // 消息类型
        cardTypes: ['任务卡'], // 卡片类型
        newsPage: 1,
        userID: '',
        allNews: 0, // 共多少条消息
...
//      返回首页
      ...
      pathPush(path1, params) { // 直接点击跳转会watch和mounted都会执行，但直接输入路径置只会执行mounted中的调用
        ...
        if (this.$route.path === '/myTesk/myTeskPublish' || this.$route.path === '/myTesk/H5share') {
          $(".inform").eq(0).css({'height': 0, 'padding-bottom': '0', 'overflow': 'hidden'});
        }
      ...
      infomShow() {
        ...
        $(".inform").eq(0).animate({height: "100%"}, 300, function () {
          $(".inform").eq(0).css({'padding-bottom': '25px', 'overflow': 'visible'});
        });
      },
      colseInform() {
        // this.infShow = false;
        $(".inform").eq(0).animate({height: "0"}, 300, function () {
          $(".inform").eq(0).css({'padding-bottom': '0', 'overflow': 'hidden'});
        });
      ...
      newsClick(news, index) {
        ...
        if (news.Enabled) {
          ...
          this.$HTTP('post', '/UpdateNotificationState', obj).then(res => {
            if (res.result) {
      ...
      allRead() {
        ...
        this.$HTTP('post', '/UpdateAllNotificationState', obj).then(res => {
          this.unreadNum = 0;
      ...
      getNewsList(page) {
        ...
        this.$HTTP('post', '/GetNotificationList', obj).then(res => {
