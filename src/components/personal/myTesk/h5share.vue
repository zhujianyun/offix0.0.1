    ...
    <div class="bottomMain">
          ...
          <el-tabs :tab-position="tabPosition" style="width: 300px;">
            ...
            <el-tab-pane label="背景">
                ...
                <div class="seTbackgroundColor">
                 <div v-for="(list,index) in shareBGList" :key="index" class="md_list"
                    :class="{'md_click': index === shareBG}">
                    <img ondragstart="return false" :src="list.url" alt="" @click="changeBG(index, list.url)"/>
                    <p class="md_title">{{list.title}}</p >
                    </div>
                </div>
            </el-tab-pane>
      ...
      <ul class="optionButton">
        ...
        >
          ...
          <span></span>
          <li id="redo" @click="redo" class="iconfont icon-47"></li>
          <li id="undo" @click="undo" class="iconfont icon-48"></li>
          <span></span>
        ...
        <div class="fileList">
          <el-collapse>
            <el-collapse-item v-for="(list,index) in cooperationName" :key="index">
              ...
              <div  v-if="list && list.TaskFileList" v-for="(fileList,index) in list.TaskFileList" :key="index">
                <ul id="fileLists">
                    ...
                    <div class="box box-element ui-draggable">
                        ...
                        >
                          <li
                            ...
                            @dragstart="fileDragStar(fileList, fileList.dragFile)"
                          >
                        ...
                        <span class="fileSelect1">{{fileList.dragFile ? '未选' : '已选'}}</span>
                      </li>
...
export default {
  ...
  data() {
    return {
      ...
      shareBGList: [
        { url: require("../../../assets/img/share_bg0.jpg"), title: "默认" },
        { url: require("../../../assets/img/share_bg1.jpg"), title: "背景1" },
        { url: require("../../../assets/img/share_bg2.jpg"), title: "背景2" },
        { url: require("../../../assets/img/share_bg3.jpg"), title: "背景3" },
        { url: require("../../../assets/img/share_bg4.jpg"), title: "背景4" },
        { url: require("../../../assets/img/share_bg5.jpg"), title: "背景5" },
        { url: require("../../../assets/img/share_bg6.jpg"), title: "背景6" },
        { url: require("../../../assets/img/share_bg7.jpg"), title: "背景7" },
        { url: require("../../../assets/img/share_bg8.jpg"), title: "背景8" }
      ], // 分享背景图
      shareBG: 0, // 默认选择
      insertIndex: 0, // 拖拽到哪
  ...
  methods: {
    ...
    clearFile(file) {
      ...
      this.fileDelNums++;
      this.fileListIds.push(parseInt(fileId));
      this.fileDelNums = this.fileListIds.length;
      console.log(this.fileListIds, this.fileDelNums);
    },
    ...
    selectCheckbox(list, val) {
      ...
      if (list.checked == false) {
        _.undoNums++;
        _.geziid.push(parseInt(list.UserId));
        _.undoNums = _.geziid.length;
        console.log("false 执行了", _.undoNums, _.geziid);
      }
    ...
    changeBG(index, url) {
      this.shareBG = index;
      $("#showMain").css({ "background-image": "url(" + url + ")" });
      // this.undoRecord();
    },
    // 获取连接
    getLinks(sharePkid) {
    ...
    // handleSaveLayout() {
    //   // var e = $("#showMain").html();
    //   if (!this.stopsave && e != window.demoHtml) {
    //     let obj = {
    //       SharingLinksPkid: localStorage.getItem("sharePkid"),
    //       Content: e
    //     };
    //     // $.ajax({
    //     //   url: 'http://182.92.161.72:1011/AppServer2.0.asmx',
    //     //   type: 'post',
    //     //   dataType: "json",
    ...
    // 撤销
    redoLayout() {
      if (this.undoNums >= 0&&this.undoNums <= this.geziid.length) {

        // if () {
          this.undoNums++
          let index = this.cooperationName.findIndex(res => {
            return this.geziid[this.undoNums - 1] == res.UserId;
          });
          if (index != -1) {
            this.cooperationName[index].checked = !this.cooperationName[index]
              .checked;
          }
      ...
      if (data) {
        if (data.list[data.count]) {
          ...
          data.count++;
          $("#showMain").html(window.demoHtml);
          if (this.supportstorage()) {
            localStorage.setItem("layoutdatas", JSON.stringify(data));
          }
          return true;
        }
      }
      ...
      //     if (this.fileDelNums) {
      //       for (let item of this.cooperationName) {
      //         for (var j of item.TaskFileList) {
      //           if (j.Pkid == this.fileListIds[this.fileDelNums - 1]) {
      //             j.dragFile = !j.dragFile;
      //           }
      //         }
      //       }
      //       this.fileDelNums--;
    ...
    saveLayout() {
      ...
      if (data.list.length > data.count) {
        for (let i = data.count; i < data.list.length; i++) data.list[i] = null;
      }
